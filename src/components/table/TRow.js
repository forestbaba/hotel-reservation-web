    ...
    return <tr onMouseEnter={onHover} onMouseLeave={onHover}
        ...
        {Object.keys(props.row).map((cell, ndx) => {
            ...
            if (props.columns[ndx + 1].type === "custom") {
                return <td key={ndx + 1} style={props.theme.cell} onClick={props.onClick}>{props.customRenderer(props.row)}</td>;
            } else {
                return <td key={ndx + 1} style={props.theme.cell} onClick={props.onClick}>{props.row[cell]}</td>;
            }
        })}
    </tr>;
}
